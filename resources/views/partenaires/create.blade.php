...
<div class="side-container-bg d-flex justify-content-center align-items-center vh-100">
    <div class="card w-50">
        ...
        <div class="card-body">
            <form action="{{route('post-partenaire')}}" method="POST" enctype="multipart/form-data">
                @method('POST')
                @csrf
                <div class="form-group">
                    <label for="email">Votre email</label>
                    <input type="text" class="form-control" id="email" value="{{Auth::user()->email}}" readonly required>
                </div>
                <div class="form-group">
                    <label for="nom_prenom">Votre nom & prénom</label>
                    <input type="text" class="form-control" id="nom_prenom" value="{{Auth::user()->nom .','. Auth::user()->prenom}}" readonly required>
                </div>
